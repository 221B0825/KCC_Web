...
const ive = iveMembers.map((x) => {
  return new FemaleIdol(x.name, x.year);
});

const bts = btsMembers.map((x) => {
...
const idolGroups = [
  new IdolGroup("아이브", ive),
  new IdolGroup("방탄소년단", bts),
];

// console.log(idolGroups);

const country = new Country("한국", idolGroups);
console.log(country);

console.log(
  ive.map((x) => {
    if (x instanceof FemaleIdol) {
      return x.dance();
    }
  })
);
